...
"""Hybrid bench: int8-stored Linears up to an HBM byte budget, fused int4 beyond.
...
INT8_BUDGET_GB = float(sys.argv[3]) if len(sys.argv) > 3 else 9.0
...
def main() -> int:
    ...
    for name, mod in list(model.named_modules()):
        ...
        cls = (Int8W4A16Linear if want_int8 else FusedW4A16Linear) if tileable else W4A16Linear
        ...
        if isinstance(new, Int8W4A16Linear):
    ...
    print(text.strip())
    print("-" * 60)
    print(f"MARKER HYBRID w4a16 compiled decode (SEQ={SEQ}, batch={BATCH}, no KV cache): "
