...
"""Full-model decode bench: gemma-4 QAT w4a16-ct with the fused Pallas
dequant-matmul kernel (nibble-plane layout) replacing every CompressedLinear.

Layers whose dims don't tile fall back to the in-graph dequant W4A16Linear.
Usage: python3.12 w4a16_fused_model_bench.py [model_id]
"""
...
BATCH = int(sys.argv[2]) if len(sys.argv) > 2 else 1  # lockstep streams, same prompt
...
class FusedW4A16Linear(torch.nn.Module):
    ...
    def forward(self, x):
        y = w4a16_matmul(x.reshape(-1, self.in_f), self.packed, self.scale_rep4)
        y = y.reshape(*x.shape[:-1], self.out_f)
        return y + self.bias if self.bias is not None else y


class W4A16Linear(torch.nn.Module):
    """In-graph dequant fallback (identical math, no Pallas)."""
...
def main() -> int:
    ...
    for name, mod in list(model.named_modules()):
        ...
        tileable = (out_f % 128 == 0 or out_f <= 512) and _ck_for(in_f) is not None and in_f % GROUP == 0
        cls = FusedW4A16Linear if tileable else W4A16Linear
        parent = model.get_submodule(name.rsplit(".", 1)[0]) if "." in name else model
        setattr(parent, name.rsplit(".", 1)[-1], cls(packed, scale, out_f, in_f, bias))
        fused += tileable
        fallback += not tileable
    print(f"MARKER swapped: {fused} fused, {fallback} fallback (in-graph)")
    ...
    print(text.strip())
    print("-" * 60)
    print(f"MARKER FUSED w4a16 compiled decode (SEQ={SEQ}, batch={BATCH}, no KV cache): "
