...
class WindowedKVTest(unittest.TestCase):
    ...
    def test_context_shorter_than_one_window(self):
        """Regression: prompt + new tokens under one window.

        build_kv_caches clamps a sliding layer's ring to min(max_seq_len, window),
        so a short context makes the ring shorter than the window. The decode mask
        has to clamp the same way; building it at full `window` yields a
        [B,1,1,window] mask that cannot broadcast against [B,H,1,ring] scores.
        Every other test here keeps the context at or above the window, so this
        path went unexercised -- it is what a short prompt hits in production.
        """
        prompt = jax.random.randint(jax.random.PRNGKey(4), (1, 1), 1, self.config.vocab_size)
        n = WINDOW - 2                      # 1 + 2 = 3 slots, under WINDOW = 4
