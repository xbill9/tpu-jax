...
class NeuronEntrypointTests(unittest.TestCase):
    def test_pallas_runs_interpreted_because_mosaic_is_tpu_only(self):
        with mock.patch.dict(os.environ, {}, clear=True):
            entrypoint.configure_neuron()
            # 0 would ask XLA to compile the Mosaic kernel natively, which no
            # Neuron backend can do; the engine must stay on the interpreter.
            self.assertEqual(os.environ["JAX_E_PALLAS_INTERPRET"], "1")
